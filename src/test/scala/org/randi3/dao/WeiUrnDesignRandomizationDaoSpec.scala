...
class WeiUrnDesignRandomizationDaoSpec extends FunSpec with MustMatchers {
  ...
  describe("WeiUrnDesignRandomizationDao create method") {
    ...
    it("should be able to create a new block randomizationmethod with parameter (alpha and beta) and without urns") {
      ...
      val id = weiUrnDesignRandomizationDao.create(urnRandomization, trialDB.id).toEither match {
        case Left(x) => fail(x)
